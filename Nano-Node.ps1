...
# Grant SYSTEM full access so Task Scheduler odometer can write to this folder
icacls "C:\nano-node" /grant "SYSTEM:(OI)(CI)F" /T | Out-Null
...
# ----------------------------------------------------------------
# Step 7 -- Download config files (skip if already exist)
# ----------------------------------------------------------------
Write-Host "Checking config files..." -ForegroundColor Cyan
if (-Not (Test-Path "C:\nano-node\Nano\config-node.toml")) {
    ...
    Write-Host "config-node.toml already exists. Skipping." -ForegroundColor Green
}
if (-Not (Test-Path "C:\nano-node\Nano\config-rpc.toml")) {
    ...
    Write-Host "config-rpc.toml already exists. Skipping." -ForegroundColor Green
}
...
$latestNanoVersion = $nanoRelease.tag_name
Write-Host "Latest release detected: $latestNanoVersion" -ForegroundColor Gray

# Check installed version
$installedVersionFile = "C:\nano-node\installed_version.txt"
if (Test-Path $installedVersionFile) {
    $installedVersion = Get-Content $installedVersionFile
    if ($installedVersion -eq $latestNanoVersion) {
        Write-Host "Nano Node $installedVersion already up to date. Skipping download." -ForegroundColor Green
        $skipNanoDownload = $true
    } else {
        Write-Host "New version available: $installedVersion -> $latestNanoVersion. Updating..." -ForegroundColor Yellow
        $skipNanoDownload = $false
    }
} else {
    $skipNanoDownload = $false
}
...
if (-Not $skipNanoDownload) {
    ...
    $latestNanoVersion | Out-File -FilePath "C:\nano-node\installed_version.txt" -Encoding UTF8
} else {
    Write-Host "Skipping Nano Node download." -ForegroundColor Green
}
...
# ----------------------------------------------------------------
# Always stop service before overwriting WinSW
$serviceBeforeWinsw = Get-Service -Name "nano-node" -ErrorAction SilentlyContinue
if ($serviceBeforeWinsw -and $serviceBeforeWinsw.Status -eq "Running") {
    Write-Host "Stopping Nano Node service temporarily..." -ForegroundColor Yellow
    Start-Process "C:\nano-node\nano-node-service.exe" -ArgumentList "stop" -Wait -NoNewWindow
    Start-Sleep -Seconds 3
    Write-Host "Service stopped." -ForegroundColor Gray
}
if (Test-Path "C:\nano-node\nano-node-service.exe") {
    Write-Host "WinSW already exists. Skipping download." -ForegroundColor Green
} else {
...
if ($choice -eq "Y" -or $choice -eq "y") {
    ...
    $dashboardInstallerUrl = "https://raw.githubusercontent.com/clixio14/Auto-Nano-Node-Windows/main/Install-Dashboard.bat"
    Invoke-WebRequest -Uri $dashboardInstallerUrl -OutFile "$env:TEMP\Install-Dashboard.bat" -UseBasicParsing
    Start-Process "$env:TEMP\Install-Dashboard.bat" -Verb RunAs
} else {
    Write-Host ""
    Write-Host "  If you change your mind, download and run the dashboard installer from:" -ForegroundColor Gray
    Write-Host "  https://github.com/clixio14/Auto-Nano-Node-Windows" -ForegroundColor Cyan
    Write-Host "  Look for: Install-Dashboard.bat" -ForegroundColor Gray
    Write-Host ""
    Write-Host "================================================================" -ForegroundColor Cyan
